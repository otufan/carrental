...
public class UserService {
    ...
    public void login(String email, String password) throws AuthException {
        ...
        try {
            ...
            if(!BCrypt.checkpw(password, user.get().getPassword())){ //Decrypt edip kontrol etmiyor... burada sifremizi database de bulunan sifre ile karsilastirip uyusmazsa hata donduruyoruz
            ...
            userRoles.forEach(role -> {
                switch (role){
                    ...
                    case "CustomerService":
                        Role customerServiceRole= roleRepository.findByName(UserRole.ROLE_CUSTOMER_SERVICE).orElseThrow(()->new RuntimeException("Error: Role is not found"));
                        roles.add(customerServiceRole);
                        break;
                    case "Manager":
                        Role managerRole= roleRepository.findByName(UserRole.ROLE_MANAGER).orElseThrow(()->new RuntimeException("Error: Role is not found"));
