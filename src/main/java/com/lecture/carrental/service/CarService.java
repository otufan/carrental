...
public class CarService {
    ...
    public void updateCar(Long id, String imageId, Car car) throws BadRequestException {
        ...
        Car carInfo=carRepository.getById(id); //findById disinda bu var. ayrica get() dememize gerek kalmadi. farkli bir sey de gormus olduk

        if (carInfo.getBuiltIn()) {
            throw new BadRequestException("You dont have permission to update car!");

        }
        // car.setBuiltIn(false); default olarak false demistik domain class inda
    ...
    public void removeById(Long id) throws BadRequestException{
        ...
        Car car=carRepository.findById(id).orElseThrow(()->new ResourceNotFoundException(String.format(CAR_NOT_FOUND_MSG, id, id)));
